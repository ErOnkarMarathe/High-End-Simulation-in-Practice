...
// Helper function to calculate the distance between two particles in 3D with periodic boundary conditions
...
__global__ void calculateForces(int numParticles, double* positions, double* forces, double sigma, double epsilon, double* domain) {
...
__global__ void calculateTotalEnergy(int numParticles, double* positions, double* velocities, double* masses, double sigma, double epsilon, double* totalEnergy,double* domain) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
int main() {
    ...
    for (int step = 0; step <= num_steps; ++step) 
    {
        ...
        if (step == 0) 
        {
        calculateForces<<<numBlocks, blockSize>>>(numParticles, d_positions, d_forces, sigma, epsilon, d_domain);
        ...
        calculateTotalEnergy<<<numBlocks, blockSize>>>(numParticles, d_positions, d_velocities, d_masses, sigma, epsilon, d_totalEnergy,d_domain);
        cudaDeviceSynchronize();
